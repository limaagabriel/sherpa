...
- **architectureRules:** a shell command that prints the project's architectural guidelines to stdout; sherpa runs it at the **plan** layer and forwards the output to `plan-breaker`, which checks the proposed plan against it. Absent → that lens falls back to an advisory general-principle check (SRP / coupling / cyclic deps / leaky abstraction, WARN-tier).
