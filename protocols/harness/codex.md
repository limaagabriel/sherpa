...
| `AskUserQuestion` | structured multiple-choice tool | Codex has **no** structured-question tool — ask **one numbered free-text question** listing the options (mark the recommended one), then wait for the reply. Same batching/serialization rules as the prose. The preamble (`protocols/questions.md`'s **The two-part shape** / **The walk**) still precedes the list; each option keeps its one-clause consequence (**The options**). |
...
- **No structured questions.** Codex's only built-in user gate is approve/deny.
  ...
  is preceded by the preamble; each option keeps its consequence clause
  (`protocols/questions.md`'s The two-part shape / The walk / The options).
