...
| `AskUserQuestion` | structured multiple-choice tool | Codex has **no** structured-question tool — ask **one numbered free-text question** listing the options (mark the recommended one), then wait for the reply. Same batching/serialization rules as the prose. A prose walk still precedes the list — *found* (what turned up, in user-observable terms), *which means* (why there's a choice), *so* (the hand-off), skipped for a pure preference question — and each option keeps its one-clause consequence, recommended option first. |
...
- **No structured questions.** Codex's only built-in user gate is approve/deny.
  ...
  one message and wait — do not fabricate a choice the user didn't make. The list
  is preceded by the prose walk (found / which means / so), skipped for a pure
  preference question; each option keeps its consequence clause, recommended
  first.
