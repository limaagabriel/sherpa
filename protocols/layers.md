...
2. **Consume the upstream artifact if it's in context; else do the *minimum* to proceed — never
   ...
   is the step layer doing a lighter discovery for its own needs, not a small `/frame`.
...
Adversarial pressure lives at each boundary — the candidate-pool critique (`/diverge`), the frame
