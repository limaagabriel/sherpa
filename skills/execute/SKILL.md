...
4. **Validate.** Follow `${CLAUDE_PLUGIN_ROOT}/protocols/workflow/phases/validate.md`: run the test plan; run the project pack's `codeStyleAudit` if one is announced (else skip — no engine style rules); dispatch `plan-breaker` `mode=output` to confirm the plan goal actually holds. One final PROGRESS rewrite with results.
5. **Turn audit.** Before claiming done, run `turn-reviewer` (`${CLAUDE_PLUGIN_ROOT}/commands/turn-review.md`) over the turn's diff + conclusions; handle the verdict per `verdict-handling.md`.
