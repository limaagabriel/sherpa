...
3. **Analyze.** Root cause + affected deps (the `## Analyze` section of `discover.md`). On assumption-failure, return to Discover.
4. **Plan.** Follow `${CLAUDE_PLUGIN_ROOT}/protocols/workflow/phases/plan.md`: present the Plan proposal (Plan-at-a-glance + before/after table + per-step blocks with goal contracts + Why/How), after a silent self-review and a `plan-breaker` `mode=briefing` pass (via Agent).
